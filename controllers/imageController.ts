...
import fs from "fs";
import { ImageModel, mongoose } from "../database/mongodb";
import { imageFilter } from "../middleware/imageMiddleware";
...
export const uploadImage = async (req: Request, res: Response) => {
  upload.single("image")(req, res, async (err) => {
    ...
    if (!req.file) {
      return res
        .status(400)
        .json({ success: false, status: 400, message: "No file uploaded." });
    }
...
export const getImage = async (req: Request, res: Response) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res
      .status(400)
      .json({ status: 400, message: "Invalid image ID", success: false });
  }
  ...
  try {
    ...
    if (!image) {
      return res
        .status(404)
        .json({ status: 404, message: "Image not found", success: false });
    }
